        ...
        if (m->audio_codec_id == AV_CODEC_ID_ADPCM_IMA_NDS ||
            ...
            if (vfield >= 0 && vfield <= (int)size) {
                if (m->audio_codec_id == AV_CODEC_ID_ADPCM_IMA_NDS) {
                     ...
                     * layout). */
                    /* vfield is the private decoder's own measured end-of-video-
                     * bitstream position, so it IS the split -- trust it as-is.
                     * (An earlier version of this code assumed vfield could
                     * overshoot by 1-2 bits and clamped it down to a block-count-
                     * derived pad estimate; that estimate assumes a fixed 4/6-byte
                     * retail pad convention that does not hold across titles --
                     * on NintendoDSi.mod it clamped nearly every frame 2-4 bytes
                     * short, corrupting the bitstream and cascading into P-frame
                     * decode failure almost immediately. Verified: with the clamp
                     * removed, NintendoDSi.mod (5073 frames) decodes with zero
                     * decoder errors.) */
                    audio_start = vfield;
